...
router.post('/', requireAuth, (req, res) => {
  ...
  if (!name || !category) return res.status(400).json({ message: '請填寫名稱與分類' })
  ...
  db.run(
    ...
    function (err) {
      ...
      res.status(201).json({ id: this.lastID, name, category, stock_quantity: qty, status: 'available' })
    }
  )
})
...
router.put('/:id', requireAuth, (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      ...
      if (this.changes === 0) return res.status(404).json({ message: '器材不存在' })
